...
def Qwen3VLModel_forward(
    ...
    **kwargs,
    ...
    if compression_on:
        ...
        patch_config = getattr(self, "_fastv_patch_config", None)
        layer_k = int(patch_config["layer_k"]) if patch_config else int(os.getenv("FASTV_K", "2"))
        retention_ratio = (
            float(patch_config["retention_ratio"])
            if patch_config
            else float(os.getenv("FASTV_R_RATIO", os.getenv("R_RATIO", "0.5")))
        )
...
def apply_fastv_patch(
    ...
    if qwen3_vl_model is None:
        raise ValueError("Unsupported Qwen3-VL model structure for FastV patching.")

    qwen3_vl_model._fastv_patch_config = {
        "layer_k": int(layer_k),
