...
def Qwen3VLModel_forward(
    ...
    **kwargs,
    ...
    if compression_on and video_attention_scores is not None:
        ...
        patch_config = getattr(self, "_holitom_patch_config", None)
        retain_ratio = (
            float(patch_config["retain_ratio"])
            if patch_config
            else float(os.getenv("HOLITOM_R_RATIO", os.getenv("R_RATIO", "0.15")))
        )
        tau = float(patch_config["tau"]) if patch_config else float(os.getenv("HOLITOM_T", "0.8"))
        beta = float(patch_config["beta"]) if patch_config else float(os.getenv("HOLITOM_BETA", "0.6"))
        D = float(patch_config["dominant_ratio"]) if patch_config else float(os.getenv("HOLITOM_D", "0.0"))
        K = int(patch_config["k_neighbors"]) if patch_config else int(os.getenv("HOLITOM_K", "7"))
        max_window_size = (
            int(patch_config["max_window_size"])
            if patch_config
            else int(os.getenv("HOLITOM_MAX_WINDOW_SIZE", "1024"))
        )
...
def apply_holitom_patch(
    ...
    if qwen3_vl_model is None:
        raise ValueError("Unsupported Qwen3-VL model structure for HoliTom patching.")

    qwen3_vl_model._holitom_patch_config = {
