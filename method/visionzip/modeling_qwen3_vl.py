...
def Qwen3VLModel_forward(
    ...
    **kwargs,
    ...
    if compression_on and video_attention_scores is not None:
        # Apply VisionZip compression with real attention scores
        patch_config = getattr(self, "_visionzip_patch_config", None)
        retention_ratio = (
            float(patch_config["retention_ratio"])
            if patch_config
            else float(os.getenv("VISIONZIP_R_RATIO", os.getenv("R_RATIO", "0.2")))
        )
        dominant_ratio = (
            float(patch_config["dominant_ratio"])
            if patch_config
            else float(os.getenv("VISIONZIP_DOMINANT_RATIO", "0.6"))
        )
        k_neighbors = (
            int(patch_config["k_neighbors"])
            if patch_config
            else int(os.getenv("VISIONZIP_K_NEIGHBORS", "5"))
        )
...
def apply_visionzip_attention_patch(
    ...
    if qwen3_vl_model is None:
        raise ValueError("Unsupported Qwen3-VL model structure for VisionZip patching.")

    qwen3_vl_model._visionzip_patch_config = {
